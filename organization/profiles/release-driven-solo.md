...
llmini の current release-driven development で使用する Operating Model を定義します。
...
- inference runtime remains package-manager-free: the serving process is `llama-server.exe` and repository maintenance tooling must not become a runtime dependency.
- `config/settings.psd1` is the model/runtime configuration authority; model URL + SHA-256 bind the expected GGUF artifact.
- downloaded llama.cpp builds, CUDA files, model files, logs, cache, and state are runtime material, not Git source.
- loopback binding is a security boundary. Exposing the API beyond 127.0.0.1 requires an explicit auth/TLS/firewall design decision.
- mise/Bun exist only to maintain project-init Agent Skills and are not required to run inference after setup.
