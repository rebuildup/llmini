...
この文書は、llmini の開発組織における最上位 contract です。
...
llmini の開発組織の目的は current procedure への服従ではありません。
...
llmini is a minimal Windows runner around downloaded llama.cpp binaries and a pinned GGUF model. Repository maintenance tooling, downloaded runtime binaries, model artifact identity, running server process, and API behavior are distinct identities.
